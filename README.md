...
短 TTL 检索缓存只缓存最终检索结果，不缓存或放宽权限判断。缓存 key 会包含当前会话、群/用户、管理员读取权限、`top_k`、时间窗口意图、分槽上限、检索模式、Embedding/Rerank 配置和记忆库 revision；记忆新增、编辑、删除、审核状态或可见性变化后，revision 会改变并自然避开旧缓存。缓存命中后日志中的检索路径会显示 `cache=hit`，未命中显示 `cache=miss`。
